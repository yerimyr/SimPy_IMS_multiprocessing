...
if __name__ == '__main__':
    ...
    for experiment_idx in range(3):
        ...
        while episode_counter < total_episodes:
            ...
            results = pool.map(worker_wrapper, tasks)  # pool.map: 모든 프로세스가 끝날 때까지 기다리고, 각각의 결과를 results 리스트에 담아줌.
            ...
            for result in results:
                ...
                main_writer.add_scalar(f"reward_core_{core_index+1}", episode_reward, global_step=episode_counter + 1)
                main_writer.add_scalar("reward_average", episode_reward, global_step=episode_counter + 1)
                episode_counter += 1
